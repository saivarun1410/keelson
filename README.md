...
- **A pattern that hits the deadline is a config bug, not a verdict about your code**, and keelson treats it exactly like any other invalid config: the hook allows the edit and prints a warning naming the pattern, `check` fails the build. That is the same split as a missing or malformed `keelson.yaml`, and it is deliberate — making CI fail open too would turn a broken pattern into silently disabled enforcement that nobody ever notices. Both sides use the same deadline, and patterns are warmed before timing, so the verdict depends on the pattern rather than on how many files happened to be scanned first.
